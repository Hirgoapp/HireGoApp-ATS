...
export class FilterLocationDto {
    @ApiPropertyOptional({ description: 'Filter by name (partial match)', example: 'New York' })
    @IsOptional()
    @IsString()
    name?: string;
    ...
    @ApiPropertyOptional({ description: 'Filter by headquarters status', example: false })
    @IsOptional()
    @IsBoolean()
    @Type(() => Boolean)
