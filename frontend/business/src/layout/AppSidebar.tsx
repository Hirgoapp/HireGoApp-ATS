...
export default function AppSidebar({ items }: AppSidebarProps) {
    ...
    const sectionOrder: NavItem['section'][] = ['overview', 'hiring', 'pipeline', 'admin'];
                        ...
                        <div className="brand-row">
                            ...
                            {!sidebarCollapsed && (
                                <div>
                                    ...
                                    <div className="brand-subtitle">Scalable Hiring Workspace</div>
                                </div>
                            )}
                        </div>
                    </div>
                                ...
                                <div className="nav-section-items">
                                    {sectionItems.map((item) => {
                                        ...
                                        if (item.status === 'coming-soon' || !item.path) {
                                            return (
                                                <div key={item.id} className="nav-item disabled" title={`${item.label} is coming soon`}>
                                                    <Icon size={16} />
                                                    {!sidebarCollapsed && (
                                                        <>
